...
logFile=open('%s\CalibrationLog_%s.txt'%(desktop,str(newtime)),'w')
selectedBand=" "
...
def findK(newframe):
    ...
    for i in range(n):
        ...
        sum_b2=(arr_sum(a2))
        #logFile.write('Second 10x10 subset of background\n')
        #writef(a2,10,10)
        #logFile.write('sum_b2= '+str(sum_b2)+'\n')
        #logFile.write('\n')
        ...
        sum_b3=(arr_sum(a3))
        #logFile.write('Third 10x10 subset of background\n')
        #writef(a3,10,10)
        #logFile.write('Sum_b3= '+str(sum_b3)+'\n')
        #logFile.write('\n')
        ...
        sum_b4=(arr_sum(a4))
        #logFile.write('Fourth 10x10 subset of background\n')
        #writef(a4,10,10)
        #logFile.write('Sum_b4= '+str(sum_b4)+'\n')
        #logFile.write('\n')
        
        sum_bm=(sum_b1+sum_b2+sum_b3+sum_b4)/400
        logFile.write('Mean background intensity= '+str(sum_bm))
        logFile.write('\n')
        print(sum)
        ...
        for i in range(len(a20)):
            a20[i]=a20[i]-sum_bm
        z=arr12(a20,20,20)    
        inter_a20=scipy.ndimage.zoom(z, 10, order=2)
        i=0
        inter_a201d=[]
        while i<200:
            j=0
            while j<200:
                inter_a201d.append(inter_a20[i][j])
                j=j+1
            i=i+1
        ...
        Pag=float(Pagr.get())
        K_VH=10*math.log10((Ip*Pag*math.sin(alph*math.pi/180))/(100*sigma))
        k_VH.append(K_VH)
...
def plotReflector(num):
    
    global intensityArray
    interpolatedArray = scipy.ndimage.zoom(intensityArray[num], 3, order=3)
    num = int(num)
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    x = y = np.arange(0, 60, 1)
    X, Y = np.meshgrid(x, y)
    zs = np.array([interpolatedArray[int(x)][int(y)] for x,y in zip(np.ravel(X), np.ravel(Y))])
...
tk.Label(mainframe, text="Angle of Incidence").grid(column=1, row=6, sticky=W)
